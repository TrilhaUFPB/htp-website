...
export function Aperture() {
  ...
  return (
    <svg className="aperture" viewBox="-800 -400 1600 800" fill="none" aria-hidden="true" focusable="false">
      <defs>
      ...
      <g>
        {bands.flatMap(({ inner, outer, offset }, band) =>
          Array.from({ length: 40 }, (_, i) => {
            ...
            return <path fill={band === 0 ? "url(#aperture-inner)" : "url(#aperture-outer)"} key={`${band}-${i}`} d={`M${point(inner, start)} L${point(outer, start)} A${outer},${outer} 0 0 1 ${point(outer, end)} L${point(inner, end)} A${inner},${inner} 0 0 0 ${point(inner, start)}Z`} />;
          }),
        )}
      </g>
    </svg>
